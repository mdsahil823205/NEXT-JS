...
const Login = () => {
  ...
  // Automatically redirect authenticated users to home page
  useEffect(() => {
  ...
  const HandleSignIn = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response) {
        setErrorMsg("No response received from authentication server. Please try again.");
        setLoading(false);
        return;
      }
      ...
      if (response.error) {
        ...
        if (
          ...
          errLower.includes("register first")
        ) {
